...
class ArgsNameTest {
    ...
    void whenKeyIsAbsent() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-=512", "request=?msg=Exit="}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
    ...
    @Test
    void whenMinusSignIsAbsent() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"dfg=512", "-request="}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument 'dfg=512' does not start with a '-' character");
    }
}